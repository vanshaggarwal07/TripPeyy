...
DROP TRIGGER IF EXISTS after_coin_update_trigger ON user_coins;
...
DROP FUNCTION IF EXISTS public.refresh_user_coin_balance(uuid);
DROP FUNCTION IF EXISTS public.update_after_purchase();
DROP FUNCTION IF EXISTS public.update_after_coin_change();

-- Drop the view if it exists
DROP VIEW IF EXISTS public.user_coin_balance;

-- Create a new view with the updated calculation
CREATE OR REPLACE VIEW public.user_coin_balance AS
SELECT 
  uc.user_id,
  uc.lifetime_earned,
  COALESCE(uc.lifetime_earned, 0) - COALESCE(up.total_redeemed, 0) AS available_coins,
  COALESCE(up.total_redeemed, 0) AS total_redeemed
FROM 
  user_coins uc
LEFT JOIN (
  SELECT 
    user_id, 
    SUM(coins_spent) as total_redeemed 
  FROM 
    user_purchases 
  GROUP BY 
    user_id
...
LANGUAGE sql
AS $$
  -- Update the user_coins table with the latest available coins
  WITH user_balance AS (
    SELECT 
      uc.user_id,
      COALESCE(uc.lifetime_earned, 0) - COALESCE(SUM(up.coins_spent), 0) AS available_coins
    FROM 
      user_coins uc
    LEFT JOIN 
...
-- Create a trigger to update available_coins when lifetime_earned changes
...
AFTER UPDATE OF lifetime_earned ON user_coins
FOR EACH ROW
...
-- Drop the total_coins column if it exists (only if it's safe to do so)
-- Note: Make sure no application code is using this column before uncommenting
-- ALTER TABLE user_coins DROP COLUMN IF EXISTS total_coins;
